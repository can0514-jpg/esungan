import type { InputState, AdviceResult, GoNoGo } from './types';
...
export async function fetchAdvice(s: InputState): Promise<AdviceResultFull> {
  ...
  const prompt = `너는 사주 명리학 30년 경력의 친절한 언니야. 아래 사주 데이터를 정확히 분석해서 조언해줘.
여성에게 말하듯 따뜻하고 명확하게. 애매한 말 금지. 구체적으로.
...
{
  "goNoGo": "go|conditional|nogo",
  "goNoGoReason": "일진 기반 판단 이유. 구체적으로 한 줄. 예: '오늘은 ${saju.ilgan}일간에게 특히 유리한 날이야'",
  "todayEnergy": "오늘 기운을 비유로 한 줄. 예: '봄비처럼 부드러운 날, 밀어붙이기보단 스며들어야 해'",
  "situation": "오늘 상황 한 줄 요약",
  "insight": "일간 특성이 오늘 상황에 작용하는 방식. 사주 용어 없이 비유로. 2줄. ~해요 체.",
  "advice": "오늘 일정에서 가장 유리한 행동 전략. 구체적인 말투·태도 포함. 3-4줄. ~해요 체.",
  "goldenTime": {
    "morning":   "오전 골든타임 + 하면 좋은 행동",
    "afternoon": "오후 골든타임 + 하면 좋은 행동",
    "evening":   "저녁 골든타임 + 하면 좋은 행동"
  },
  "luckyItem": "오늘 행운 컬러 또는 소품 추천. 이유 포함. 한 줄.",
  "checkpoints": ["오늘 반드시 할 것 1", "할 것 2", "할 것 3"],
  "bans": ["오늘 하지 말 것 1 (명리학적 이유)", "하지 말 것 2 (이유)"],
  "tension": "높음|중간|낮음",
  "advantage": "유리|불리|중립",
  "status": "상황 한두 단어",
  "tags": ["태그1", "태그2", "태그3"],
...
export function fallbackAdvice(s: InputState): AdviceResultFull {
  ...
  const byPurpose: Record<string, Pick<AdviceResultFull, 'goNoGo' | 'goNoGoReason' | 'advice' | 'checkpoints' | 'bans' | 'goldenTime' | 'luckyItem'>> = {
    '탐색/네트워킹': {
      goNoGo: 'go',
      goNoGoReason: '오늘은 새로운 연결에 유리한 기운이에요. 가도 좋아!',
      advice: `${p.talkStyle}인 성향이 오늘은 오히려 장점이에요. 상대가 먼저 말하게 유도하고, 즉답보다는 "한번 생각해볼게요"로 여지를 남겨두세요.`,
      checkpoints: ['상대 먼저 말하게 유도하기', '즉답 피하고 "생각해볼게요" 패턴 유지', '상대 관심사 파악 후 메모'],
      bans: ['연봉·이직 의사를 먼저 꺼내지 마세요 — 협상력이 사라져요', '결론 없는 말 길게 하기 — 핵심만 간결하게'],
      goldenTime: { morning: '오전 10시 전후 — 머리가 가장 맑을 때 준비하세요', afternoon: '오후 2~4시 — 이야기 흐름이 자연스럽게 열려요', evening: '저녁 6시 이후 — 가벼운 마무리 대화로 인상을 남기기 좋아요' },
      luckyItem: '남색이나 짙은 파란색 소품 — 신뢰감과 안정감을 전달해요',
    },
    '업무 보고/논의': {
      goNoGo: 'conditional',
      goNoGoReason: '가되, 결론부터 말하는 준비가 필요해요. 조건부 가세요.',
      advice: `결론부터 말하는 습관이 오늘 가장 중요해요. 문제를 얘기할 때는 해결책을 반드시 세트로 가져가세요. ${p.watchOut}을 특히 조심하세요.`,
      checkpoints: ['결론 먼저, 배경은 나중에', '문제엔 해결책 세트로 준비', '상대 말 끊지 않기'],
      bans: [`"잘 모르겠는데요" 식의 자신없는 말 — 신뢰도가 떨어져요`, `${p.watchOut} — 오늘은 특히 역효과 나요`],
      goldenTime: { morning: '오전 9~11시 — 집중력이 가장 높아요, 보고 준비 마무리하세요', afternoon: '오후 2~3시 — 의사결정이 잘 되는 시간이에요', evening: '저녁 7시 이후 — 오늘 대화 복기하고 다음 준비하기 좋아요' },
      luckyItem: '화이트나 라이트그레이 계열 — 깔끔하고 신뢰감 있는 인상을 줘요',
    },
    '부탁/협상': {
      goNoGo: 'go',
      goNoGoReason: '오늘 협상 기운이 열려 있어요. 망설이지 말고 가세요!',
      advice: `이유를 여러 개 나열하면 오히려 설득력이 약해져요. 가장 강한 이유 하나만 써서 간결하게 전달하세요.`,
      checkpoints: ['가장 강한 이유 하나만 선택', '상대 상황 먼저 챙기는 말로 시작', '부탁은 짧고 명확하게'],
      bans: ['미리 포기하는 말 꺼내지 마세요 — 협상력이 0이 돼요', '이유를 3개 이상 나열하기 — 핵심이 흐려져요'],
      goldenTime: { morning: '오전은 준비에 집중하세요', afternoon: '오후 1~3시 — 상대가 수용적인 상태가 되는 시간이에요', evening: '저녁은 마무리 분위기 — 결론이 안 났으면 다음 기회를 잡으세요' },
      luckyItem: '따뜻한 베이지나 카멜 톤 — 친근하면서도 진지한 인상을 줘요',
    },
    '갈등/해결': {
      goNoGo: 'conditional',
      goNoGoReason: '감정을 가라앉히고 가면 잘 풀려요. 조건부 가세요.',
      advice: `상대 입장을 먼저 인정하는 말로 시작하는 게 핵심이에요. 감정이 올라올 때는 결론을 내지 마세요.`,
    ...
    '그냥 친목': {
      goNoGo: 'go',
      goNoGoReason: '오늘 친목 기운 최고! 부담 없이 가도 좋아요 😊',
      advice: `업무 얘기는 전체 대화의 30%를 넘기지 마세요. 상대방의 관심사를 먼저 물어보고 잘 들어주는 것만으로도 좋은 인상을 남겨요.`,
      checkpoints: ['업무 얘기 30% 이하로 제한', '상대 관심사 한 가지 물어보기', '폰 내려놓고 눈 맞추기'],
      bans: ['끝없는 회사·업무 불만 얘기 — 분위기가 무거워져요', '자기 얘기만 계속하기 — 상대가 지루해해요'],
      goldenTime: { morning: '오전엔 컨디션 관리', afternoon: '오후 2~6시 — 대화가 자연스럽게 흐르는 골든타임이에요', evening: '저녁 7시 이후 — 분위기가 가장 무르익는 시간이에요' },
      luckyItem: '밝은 오렌지나 노란색 소품 — 활기차고 친근한 인상을 줘요',
    },
    '첫 만남': {
      goNoGo: 'go',
      goNoGoReason: '첫 만남에 좋은 날이에요. 자신 있게 가세요!',
      advice: `첫인상은 처음 3분에서 결정돼요. 말을 잘하는 것보다 잘 들어주는 게 훨씬 좋은 인상을 남겨요.`,
      checkpoints: ['첫 3분 집중 — 미소와 눈 맞춤', '상대 이름 대화 중 한 번 불러주기', '자기 자랑 자제하고 질문 위주로'],
      bans: ['과도한 자기 자랑 — 첫 만남에서 역효과예요', '너무 많은 질문 한꺼번에 — 심문받는 느낌이 들어요'],
      goldenTime: { morning: '오전엔 외모와 복장 준비', afternoon: '오후 1~4시 — 긴장이 풀리고 자연스러운 대화가 나오는 시간이에요', evening: '저녁은 마무리의 기회 — 다음 약속으로 이어지기 좋아요' },
  ...
  return {
    ...
    advantage:   '유리',
    status:      s.purpose,
    tags:        [s.occupation, s.place, s.purpose].filter(Boolean),
    ...fb,
  };
}
